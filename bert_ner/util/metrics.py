...
def get_chunk(sequence, label_to_index,query=""):
    """
    ...
    for index, label in enumerate(sequence):
        if label in unentry:
            ...
            if chunk_type is None:
                ...
                if query!="":
                    chunk = (query[chunk_start:index], chunk_type, chunk_start, index-1)
                else:
        ...
        if label not in unentry:
            ...
            if chunk_type is None:
                if label_chunk_class not in ["I","E"]:
                ...
                if index == (len(sequence) - 1):
                    ...
                    if query!="":
                        chunk = (query[chunk_start:index], chunk_type, chunk_start, index)
                    else:
                ...
                if query!="":
                    chunk = (query[chunk_start-1:index-1], chunk_type, chunk_start, index-1)
                else:
                    chunk = (chunk_type, chunk_start, index-1)
                chunks.append(chunk)
                chunk_type, chunk_start = label_chunk_type, index
