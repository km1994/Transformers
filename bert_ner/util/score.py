...
def get_tags(paths, tag_map):
    tags = []
    print(f"paths:{paths}") 
    for path in paths:
        tags.append(tag_map[path])
    return tags
...
def f1_score(tar_path, pre_path, tag, type):
    ...
    for fetch in zip(tar_path, pre_path):
        tar, pre = fetch
        if type == "BIO":
            tar_tags = get_tags_BIO(tar, tag, args.tag_map)
            pre_tags = get_tags_BIO(pre, tag, args.tag_map)
        elif type == "BIEO":
            tar_tags = get_tags_BIEO(tar, tag, args.tag_map)
            pre_tags = get_tags_BIEO(pre, tag, args.tag_map)
        elif type == "BIESO":
            tar_tags = get_tags_BIESO(tar, tag, args.tag_map)
            pre_tags = get_tags_BIESO(pre, tag, args.tag_map)
        # print(tar_tags)
        # print(pre_tags)
        origin += len(tar_tags)
        found += len(pre_tags)
