...
class ProgressBar(object):
    ...
    # 显示函数，根据当前的处理进度i显示进度
    # 效果为[>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>]100.00%
    def show_process(self, train_acc, train_loss, f1, entity_p, entity_r, entity_f, used_time, i):
        ...
        process_bar = '%d' % num_steps + '/' + '%d' % self.epoch_size + '[' + '>' * num_arrow + '-' * num_line + ']' \
                      ...
                      '%.4f' % train_loss + ' - f1 ' + '%.4f' % f1 + ' - ep ' + '%.4f' % entity_p + ' - er ' + '%.4f' % entity_r + ' - ef ' + '%.4f' % entity_f + ' - time ' + '%.1fs' % used_time + '\r'
        sys.stdout.write(process_bar)  # 这两句打印字符到终端
        sys.stdout.flush()

        # 显示函数，根据当前的处理进度i显示进度
    # 效果为[>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>]100.00%
    def show_process1(self, train_acc, train_loss, p, r, f, used_time, i):
        num_arrow = int(i * self.max_arrow / self.max_steps)  # 计算显示多少个'>'
        num_line = self.max_arrow - num_arrow  # 计算显示多少个'-'
        percent = i * 100.0 / self.max_steps  # 计算完成进度，格式为xx.xx%
        num_steps = self.batch_size * i  # 当前处理数据条数
        process_bar = '%d' % num_steps + '/' + '%d' % self.epoch_size + '[' + '>' * num_arrow + '-' * num_line + ']' \
                      + '%.2f' % percent + '%' + ' - train_acc ' + '%.4f' % train_acc + ' - train_loss ' + \
                      '%.4f' % train_loss + ' - p ' + '%.4f' % p + ' - r ' + '%.4f' % r + ' - f ' + '%.4f' % f + ' - time ' + '%.1fs' % used_time + '\r'
        sys.stdout.write(process_bar)  # 这两句打印字符到终端
        sys.stdout.flush()
