...
from DataPrecessForSentencePair import DataPrecessForSentencePair
from LabelSmoothing import LabelSmoothing

if __name__=="__main__":
    config = Config()
    # 训练集加载
    train_data = load_data(config.data_path,config.data_file_list[config.data_index],config.col_list, sep=config.sep)
    # 标签映射表构建
    label2id_dic,id2label_dic = build_label2id(train_data,config.data_path)
    print(f"label2id_dic:{label2id_dic}")
    print(f"id2label_dic:{id2label_dic}")
    exit(0)
    train_data = train_data[config.col_list[0:-2]+config.col_list[-1:]]
    # 训练数据集拆分为训练集和验证集
    ...
    train_labels = train.groupby([config.col_list[-1]]).count().reset_index()
    valid_labels = valid.groupby([config.col_list[-1]]).count().reset_index()

    # 分词工具
    bert_tokenizer = BertTokenizer.from_pretrained(config.bert_model_path, do_lower_case=config.do_lower_case)
    # 类初始化
    processor = DataPrecessForSentencePair(bert_tokenizer= bert_tokenizer)
    # 加载预训练的bert模型
